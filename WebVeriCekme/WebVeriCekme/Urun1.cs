    ...
    class Urun1
    {
        internal static void Urun_1()
        {
            ...
            try
            {
                ...
                //Fiyat aralığını almak için oluşturuan parametre
                ...
                //Ürünlerden 1.si
                string sqlUrunBilgileri = "Select UrunAdi,GoruntulenmeSayisi,Fiyat from UrunBilgileri where UrunAdi='Nivea Duş Jeli Deep Dimension 500 ml'";
                ...
                if (DataSetKontrol(dsUrunBilgileri))
                {
                    ...
                    if ((YeniUrunFiyatiD - EskiUrunFiyatiD) < Convert.ToDouble(farkdeger) || (YeniUrunFiyatiD - EskiUrunFiyatiD) > Convert.ToDouble(farkdeger))//Aradaki far parametrede belirlenen degerden farklıysa kontrolü yapılır
                    {
                        command = new SqlCommand();

                        command.Connection = connection;
                        command.CommandText = "update UrunBilgileri set  GoruntulenmeSayisi= @GoruntulenmeSayisi ,Fiyat=@Fiyat where UrunAdi='Nivea Duş Jeli Deep Dimension 500 ml'";
                ...
                {
                    //Ürün veritabanında hiç yokken bu blok çalışacak
                    command = new SqlCommand();
                    ...
                    command.Parameters.AddWithValue("@UrunAdi", "Nivea Duş Jeli Deep Dimension 500 ml");
            ...
            {
                Console.WriteLine("Hata1: " + ex.Message);
            }
        }// Ürün bilgilerini database kaydeden kod bloğu
        public static string FiyatiniOku()
        {
            try
            {
                var url = new Uri("https://www.rossmann.com.tr/nivea-dus-jeli-deep-dimension-500-ml-p-st18050587"); // url oluştruduk
            ...
            {
                Console.WriteLine("Hata2.1: " + ex.Message);
            }

            return "";
        }// Ürünün fiyat bilgisini çeken kod bloğu
        ...
        public static string GoruntulenmeSayisiniOku()
        {
            try
            {
                var url = new Uri("https://www.rossmann.com.tr/davidoff-cool-water-edt-erkek-parfum-125ml-p-st11060143"); // url oluştruduk
                var client = new WebClient(); // siteye erişim için client tanımladık
                var html = client.DownloadString(url); //sitenin html lini indirdik
                HtmlAgilityPack.HtmlDocument doc = new HtmlAgilityPack.HtmlDocument(); //burada HtmlAgilityPack Kütüphanesini kullandık
                doc.LoadHtml(html); // indirdiğimiz sitenin html lini oluşturduğumuz dokumana dolduruyoruz
            ...
            return "0";
        }// Goruntulenme sayısını çekilemiyor.
        ...
        }// Gmail bireysel hesap güvenlik hatasından mail attırmıyor.
    }
}
